 ...
 * oauth_client.php
 *
 * @(#) $Id: oauth_client.php,v 1.63 2013/06/22 12:22:03 mlemos Exp $
 *
 */
...
<class>
	...
	<version>@(#) $Id: oauth_client.php,v 1.63 2013/06/22 12:22:03 mlemos Exp $</version>
...
*/
	...
	var $oauth_user_agent = 'PHP-OAuth-API (http://www.phpclasses.org/oauth-api $Revision: 1.63 $)';
...
*/
	...
	Function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response)
	{
		...
		&& !strcasecmp($this->access_token_type, 'Bearer'))
			$authorization = 'Bearer '.UrlEncode($this->access_token);
		...
		switch($content_type)
		{
			...
			case 'application/json':
				...
				$object = json_decode($data, $options['AsArray']);
...
*/
	Function Initialize()
	{
		...
		switch($this->server)
		{
            ...
            case 'Quag':
                $this->oauth_version = '2.0';
                $this->dialog_url = 'https://www.quentrix.com/oauth2/authorize/?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&scope={SCOPE}&state={STATE}';
                $this->access_token_url = 'https://www.quentrix.com/oauth2/token/';
                break;     
